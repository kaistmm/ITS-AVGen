#!/bin/bash
# Standard inference script (No ITS)
# Usage: bash scripts/inference_standard.sh [GPU_ID] [NSHARD] [SHARD_ID]
# Examples:
...
cfg_file="configs/javisdit-v0-1/inference/sample_240p4s_standard.py"
save_dir="samples/generated_standard_$(date +%Y%m%d_%H%M%S)"
...
echo "Running Standard inference (No ITS)"
